                ...
                <template slot-scope="scope">
                    <el-button-group>
                        ...
                        <el-button :title="(scope.row.delete ? '启用':'禁用') +'码本分组'"  size="mini" type="primary"
                                   :icon="scope.row.delete ? 'el-icon-fa-recycle': 'el-icon-fa-trash'" @click="handleDelete(scope.$index,scope.row)"></el-button>
                    </el-button-group>
                </template>
            </el-table-column>
        </el-table>
        ...
        <el-dialog :title="Group.id == 0 ? '添加码本分组' : '编辑码本分组' " :visible.sync="addEditShow" width="30%">
            <el-form :model="Group" :rules="$rules" ref="GroupForm">
                <el-form-item label="名称" :label-width="formLabelWidth" prop="name">
                    <el-input v-model="Group.name" auto-complete="off" placeholder="请填写码本分组名称" suffix-icon="el-icon-fa-vcard" ></el-input>
                </el-form-item>
                <el-form-item label="描述" :label-width="formLabelWidth" prop="description">
                    <el-input type="textarea" :maxlength="500"
                    :autosize="{ minRows: 4, maxRows: 8}" v-model="Group.description" auto-complete="off" placeholder="请填写码本分组描述" suffix-icon="el-icon-fa-file-word-o"></el-input>
                </el-form-item>
            </el-form>
